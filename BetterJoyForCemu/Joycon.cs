...
namespace BetterJoyForCemu
{
    public class Joycon
    {
        ...
        private readonly int _highFreq = int.Parse(ConfigurationManager.AppSettings["HighFreqRumble"]);
        private readonly bool imu_enabled;
        public bool isLeft;
        public bool isPro;
        private readonly bool isUSB;

        public byte LED = 0x0;
        private readonly short[] left_hor_offset = {0, 0, 0};
        private readonly int lowFreq = int.Parse(ConfigurationManager.AppSettings["LowFreqRumble"]);
        public int model = 2;
        public Joycon other;
        ...
        private readonly int rumblePeriod = int.Parse(ConfigurationManager.AppSettings["RumblePeriod"]);
        ...
        private readonly bool swapButtons = bool.Parse(ConfigurationManager.AppSettings["SwapButtons"]);
        private int timestamp;
        public ulong Timestamp;
        private const float timing = 120.0f;
        ...
        public void DebugPrint(string s, DebugType d)
        {
            ...
            if (d == DebugType.ALL || d == debug_type || debug_type == DebugType.ALL) form.AppendTextBox(s + "\r\n");
        }
        ...
        public int Attach(byte leds_ = 0x0)
        {
            ...
            else
            {
                ...
                if (a[2] != 0x3) PadMacAddress = new PhysicalAddress(new[] {a[9], a[8], a[7], a[6], a[5], a[4]});
                ...
                a[0] = 0x80;
                a[1] = 0x03; // 3Mbit baud rate
                HIDapi.hid_write(handle, a, new UIntPtr(2));

                a[0] = 0x80;
                a[1] = 0x02; // Handshake at new baud rate
                HIDapi.hid_write(handle, a, new UIntPtr(2));

                a[0] = 0x80;
                a[1] = 0x04; // Prevent HID timeout
                HIDapi.hid_write(handle, a, new UIntPtr(2));
            ...
            a[0] = leds_;
            Subcommand(0x30, a, 1);
            Subcommand(0x40, new[] {imu_enabled ? (byte) 0x1 : (byte) 0x0}, 1, true);
        ...
        public void BlinkLED()
        {
            ...
            var a = Enumerable.Repeat((byte) 0xFF, 25).ToArray(); // LED ring
            a[0] = 0xFF;
            a[1] = 0x01;
        ...
        public void Detach()
        {
            ...
            if (State > state_.NO_JOYCONS)
            {
                ...
                if (isUSB)
                {
                    ...
                    a[0] = 0x80;
                    a[1] = 0x05; // Allow device to talk to BT again
                    HIDapi.hid_write(handle, a, new UIntPtr(2));
                    a[0] = 0x80;
                    a[1] = 0x06; // Allow device to talk to BT again
                    HIDapi.hid_write(handle, a, new UIntPtr(2));
                }
            }

            if (State > state_.DROPPED) HIDapi.hid_close(handle);
        ...
        public void Update()
        {
            if (State > state_.NO_JOYCONS)
                ...
                {
                    if (rumble_obj.t < 0)
                        rumble_obj.set_vals(lowFreq, _highFreq, 0, 0);
                    else
        ...
        private int ProcessButtonsAndStick(byte[] report_buf)
        {
            ...
            lock (buttons)
            {
                lock (down_)
                {
                    for (var i = 0; i < buttons.Length; ++i) down_[i] = buttons[i];
                }
                ...
                if (!isLeft && other != null) buttons[(int) Button.MINUS] = other.buttons[(int) Button.MINUS];

                if (!isPro && xin != null)
                {
                    if (other != null)
                    {
                        report.SetButtonState(!swapButtons ? Xbox360Buttons.A : Xbox360Buttons.B,
                            buttons[(int) (isLeft ? Button.B : Button.DPAD_DOWN)]);
                        report.SetButtonState(!swapButtons ? Xbox360Buttons.B : Xbox360Buttons.A,
                            buttons[(int) (isLeft ? Button.A : Button.DPAD_RIGHT)]);
                        report.SetButtonState(!swapButtons ? Xbox360Buttons.Y : Xbox360Buttons.X,
                            buttons[(int) (isLeft ? Button.X : Button.DPAD_UP)]);
                        report.SetButtonState(!swapButtons ? Xbox360Buttons.X : Xbox360Buttons.Y,
                        ...
                        report.SetButtonState(Xbox360Buttons.Down,
                            buttons[(int) (isLeft ? Button.DPAD_DOWN : Button.B)]);
                        report.SetButtonState(Xbox360Buttons.Left,
                            buttons[(int) (isLeft ? Button.DPAD_LEFT : Button.Y)]);
                        report.SetButtonState(Xbox360Buttons.Right,
                            buttons[(int) (isLeft ? Button.DPAD_RIGHT : Button.A)]);
                        report.SetButtonState(Xbox360Buttons.Back, buttons[(int) Button.MINUS]);
                        report.SetButtonState(Xbox360Buttons.Start, buttons[(int) Button.PLUS]);
                        report.SetButtonState(Xbox360Buttons.Guide, buttons[(int) Button.HOME]);
                        report.SetButtonState(Xbox360Buttons.LeftShoulder,
                            buttons[(int) (isLeft ? Button.SHOULDER_1 : Button.SHOULDER2_1)]);
                        report.SetButtonState(Xbox360Buttons.RightShoulder,
                            buttons[(int) (isLeft ? Button.SHOULDER2_1 : Button.SHOULDER_1)]);
                        report.SetButtonState(Xbox360Buttons.LeftThumb,
                            buttons[(int) (isLeft ? Button.STICK : Button.STICK2)]);
                        report.SetButtonState(Xbox360Buttons.RightThumb,
                            buttons[(int) (isLeft ? Button.STICK2 : Button.STICK)]);
                    }
                    else
                    {
                        // single joycon mode
                        ...
                        report.SetButtonState(Xbox360Buttons.Back,
                            buttons[(int) Button.MINUS] | buttons[(int) Button.HOME]);
                        report.SetButtonState(Xbox360Buttons.Start,
                            buttons[(int) Button.PLUS] | buttons[(int) Button.CAPTURE]);
            ...
            if (xin == null) return 0;
            if ((other != null) | isPro)
            {
                report.SetAxis(Xbox360Axes.LeftThumbX,
                    (short) Math.Max(short.MinValue,
                        Math.Min(short.MaxValue, stick[0] * (stick[0] > 0 ? short.MaxValue : -short.MinValue))));
                report.SetAxis(Xbox360Axes.LeftThumbY,
                    (short) Math.Max(short.MinValue,
                        Math.Min(short.MaxValue, stick[1] * (stick[1] > 0 ? short.MaxValue : -short.MinValue))));
                report.SetAxis(Xbox360Axes.RightThumbX,
                    (short) Math.Max(short.MinValue,
                        Math.Min(short.MaxValue, stick2[0] * (stick2[0] > 0 ? short.MaxValue : -short.MinValue))));
                report.SetAxis(Xbox360Axes.RightThumbY,
                    (short) Math.Max(short.MinValue,
                        Math.Min(short.MaxValue, stick2[1] * (stick2[1] > 0 ? short.MaxValue : -short.MinValue))));
            }
            else
            {
                // single joycon mode
                report.SetAxis(Xbox360Axes.LeftThumbY,
                    (short) ((isLeft ? 1 : -1) * Math.Max(short.MinValue,
                                 Math.Min(short.MaxValue,
                                     stick[0] * (stick[0] > 0 ? short.MaxValue : -short.MinValue)))));
                report.SetAxis(Xbox360Axes.LeftThumbX,
                    (short) ((isLeft ? -1 : 1) * Math.Max(short.MinValue,
                                 Math.Min(short.MaxValue,
                                     stick[1] * (stick[1] > 0 ? short.MaxValue : -short.MinValue)))));
            }

            report.SetAxis(Xbox360Axes.LeftTrigger,
                (short) (buttons[(int) (isLeft ? Button.SHOULDER_2 : Button.SHOULDER2_2)] ? short.MaxValue : 0));
            report.SetAxis(Xbox360Axes.RightTrigger,
                (short) (buttons[(int) (isLeft ? Button.SHOULDER2_2 : Button.SHOULDER_2)] ? short.MaxValue : 0));
            ...
            for (var i = 0; i < 3; ++i)
                switch (i)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        acc_g.Y = (!isLeft ? -1 : 1) * (acc_r[i] - offset[i]) *
                                  (1.0f / (acc_sensiti[i] - acc_neutral[i])) * 4.0f;
                        gyr_g.Y = -(!isLeft ? -1 : 1) * (gyr_r[i] - gyr_neutral[i]) *
                                  (816.0f / (gyr_sensiti[i] - gyr_neutral[i]));
                        break;
                    case 2:
                        acc_g.Z = (!isLeft ? -1 : 1) * (acc_r[i] - offset[i]) *
                                  (1.0f / (acc_sensiti[i] - acc_neutral[i])) * 4.0f;
                        gyr_g.Z = -(!isLeft ? -1 : 1) * (gyr_r[i] - gyr_neutral[i]) *
                                  (816.0f / (gyr_sensiti[i] - gyr_neutral[i]));
                        break;
                }

            //foreach (var test in report_buf) Console.Write(((short) test).ToString());
            //Console.Write("\n");

            if (other != null || isPro) return;
        ...
        public void Begin()
        {
            ...
            {
                PollThreadObj = new Thread(Poll) {IsBackground = true};
                PollThreadObj.Start();
        ...
        private static float[] CenterSticks(ushort[] vals, ushort[] cal, ushort dz)
        {
            var t = cal;
        ...
        {
            var buf_ = ReadSPI(0x80, isLeft ? (byte) 0x12 : (byte) 0x1d, 9); // get user calibration data if possible
            var found = false;
            for (var i = 0; i < 9; ++i)
                if (buf_[i] != 0xff)
                {
            ...
            if (isPro)
            {
                ...
                {
                    form.AppendTextBox("Using factory stick calibration data.\r\n");
                    buf_ = ReadSPI(0x60, !isLeft ? (byte) 0x3d : (byte) 0x46,
                        9); // get user calibration data if possible
                }
        ...
        private byte[] ReadSPI(byte addr1, byte addr2, uint len, bool print = false)
        {
            ...
            for (var i = 0; i < 100; ++i)
            {
                ...
                if (buf_[15] == addr2 && buf_[16] == addr1) break;
            }
        ...
        private struct Rumble
        {
            ...
            public byte[] GetData()
            {
                ...
                else
                {
                    ...
                    var hf = (ushort) ((Math.Round(32f * Math.Log(h_f * 0.1f, 2)) - 0x60) * 4);
                    var lf = (byte) (Math.Round(32f * Math.Log(l_f * 0.1f, 2)) - 0x40);
                    byte hf_amp;
                    if (Math.Abs(amp) < 1) hf_amp = 0;
                    else if (amp < 0.117)
                        hf_amp = (byte) ((Math.Log(amp * 1000, 2) * 32 - 0x60) / (5 - Math.Pow(amp, 2)) - 1);
